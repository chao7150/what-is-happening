...
{
	if (region.at.Z === region.az.Z)
	{
		return 'on line ' + region.at.Z;
	}
	return 'on lines ' + region.at.Z + ' through ' + region.az.Z;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { bc: 'hidden', a7: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { bc: 'mozHidden', a7: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { bc: 'msHidden', a7: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { bc: 'webkitHidden', a7: 'webkitvisibilitychange' }
		: { bc: 'hidden', a7: 'visibilitychange' };
}
...
var $MartinSStewart$elm_audio$Audio$audioDefaultConfig = {z: $elm$core$Maybe$Nothing, B: 1, u: $ianmackenzie$elm_units$Quantity$zero};
...
var $MartinSStewart$elm_audio$Audio$audioWithConfig = F3(
	function (audioSettings, source, startTime) {
		return $MartinSStewart$elm_audio$Audio$BasicAudio(
			{aV: audioSettings, F: source, aa: startTime});
	});
...
var $author$project$Main$audio = F2(
	function (_v0, model) {
		if (model.$ === 1) {
			...
			var _v2 = loadedModel._;
			if (!_v2.$) {
				return $MartinSStewart$elm_audio$Audio$silence;
			} else {
				var time = _v2.a;
				return A2($MartinSStewart$elm_audio$Audio$audio, loadedModel.as, time);
			}
		} else {
			return $MartinSStewart$elm_audio$Audio$silence;
		}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aB: fragment, aD: host, aJ: path, aL: port_, aO: protocol, aP: query};
	});
...
var $MartinSStewart$elm_audio$Audio$audioStartTime = function (audio_) {
	return A2($ianmackenzie$elm_units$Duration$addTo, audio_.aa, audio_.N);
};
		...
		return $elm$json$Json$Encode$object(
			_List_fromArray(
				[
					_Utils_Tuple2(
					'loopStart',
					$MartinSStewart$elm_audio$Audio$encodeDuration(loop.aG)),
					_Utils_Tuple2(
					'loopEnd',
			...
			_List_fromArray(
				[
					...
					$MartinSStewart$elm_audio$Audio$encodeBufferId(
						$MartinSStewart$elm_audio$Audio$audioSourceBufferId(audio_.F))),
					_Utils_Tuple2(
					'startTime',
					...
					_Utils_Tuple2(
					'volume',
					$elm$json$Json$Encode$float(audio_.S)),
					_Utils_Tuple2(
					'volumeTimelines',
					A2(
						$elm$json$Json$Encode$list,
					...
					_Utils_Tuple2(
					'playbackRate',
					$elm$json$Json$Encode$float(audio_.B))
				]));
	});
...
var $MartinSStewart$elm_audio$Audio$flattenAudio = function (audio_) {
	switch (audio_.$) {
		...
		case 1:
			...
			var startTime = audio_.a.aa;
			var settings = audio_.a.aV;
			return _List_fromArray(
				[
					{z: settings.z, N: $ianmackenzie$elm_units$Quantity$zero, B: settings.B, F: source, u: settings.u, aa: startTime, S: 1, ah: _List_Nil}
				]);
		default:
			var effect = audio_.a;
			var _v1 = effect.ak;
					...
					return A2(
						...
						function (a) {
							return _Utils_update(
								a,
								{S: scaleVolume_.aS * a.S});
						},
						$MartinSStewart$elm_audio$Audio$flattenAudio(effect.a5));
				case 1:
					var volumeAt = _v1.a.a$;
					return A2(
						$elm$core$List$map,
						function (a) {
							return _Utils_update(
...
var $MartinSStewart$elm_audio$Audio$updateAudioState = F2(
	function (_v0, _v1) {
		...
		var validAudio = A2(
			...
			function (_v7) {
				var a = _v7.b;
				return _Utils_eq(a.F, audioGroup.F) && (_Utils_eq(
					$MartinSStewart$elm_audio$Audio$audioStartTime(a),
					$MartinSStewart$elm_audio$Audio$audioStartTime(audioGroup)) && _Utils_eq(a.u, audioGroup.u));
			},
		...
		return $elm$json$Json$Encode$object(
			_List_fromArray(
				[
					_Utils_Tuple2(
					'audioUrl',
					$elm$json$Json$Encode$string(audioLoad.U)),
					_Utils_Tuple2(
					'requestId',
		...
		var newPendingRequests = A2(
			...
			F2(
				function (index, request) {
					return _Utils_Tuple2(model.D + index, request);
				}),
			flattenedAudioCmd);
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					...
					D: model.D + $elm$core$List$length(flattenedAudioCmd)
				}),
			A2(
				$elm$json$Json$Encode$list,
				$elm$core$Basics$identity,
...
var $MartinSStewart$elm_audio$Audio$initHelper = F3(
	function (audioPort, audioFunc, _v0) {
		...
		var json = _v1.c;
		var initialModel = {H: audioState, M: newNodeGroupIdCounter, e: $elm$core$Dict$empty, D: 0, E: $elm$core$Maybe$Nothing, k: $elm$core$Dict$empty, w: model};
					...
					F3(
						function (requestId, bufferId, duration) {
							return $MartinSStewart$elm_audio$Audio$AudioLoadSuccess(
								{
									V: bufferId,
									X: $ianmackenzie$elm_units$Duration$seconds(duration),
									an: requestId
								});
						}),
					A2($elm$json$Json$Decode$field, 'requestId', $elm$json$Json$Decode$int),
			...
			default:
				return $elm$json$Json$Decode$succeed(
					$MartinSStewart$elm_audio$Audio$JsonParseError(
						{
							Y: 'Type ' + ($elm$core$String$fromInt(value) + ' not handled.')
						}));
		}
	},
...
var $MartinSStewart$elm_audio$Audio$fromJSPortSub = function (json) {
	...
	if (!_v0.$) {
		...
		return $MartinSStewart$elm_audio$Audio$FromJSMsg(
			$MartinSStewart$elm_audio$Audio$JsonParseError(
				{
					Y: $elm$json$Json$Decode$errorToString(error)
				}));
	}
};
			...
			_List_fromArray(
				[
					...
					A2(
						...
						model.w)),
					app.a6.bb($MartinSStewart$elm_audio$Audio$fromJSPortSub)
				]));
	});
...
var $MartinSStewart$elm_audio$Audio$updateHelper = F4(
	function (audioPort, audioFunc, userUpdate, _v0) {
		...
		var _v1 = A2(userUpdate, audioData_, model.w);
		...
		var newModel = _Utils_update(
			model,
			{H: audioState, M: newNodeGroupIdCounter, w: newUserModel});
	...
	function (app, msg, _v0) {
		...
		if (msg.$ === 1) {
			...
			return A4(
				...
				app.a6.bk,
				app.a5,
				A2($MartinSStewart$elm_audio$Audio$flip, app.bl, userMsg),
				model);
		} else {
			var response = msg.a;
			switch (response.$) {
				case 0:
					var requestId = response.a.an;
					var bufferId = response.a.V;
					var duration = response.a.X;
					...
					if (!_v3.$) {
						...
						var maybeUserMsg = A2(
							...
							$mgold$elm_nonempty_list$List$Nonempty$toList(pendingRequest.x));
						if (!maybeUserMsg.$) {
							...
							return A4(
								...
								_Utils_update(
									model,
									{
										e: A2($elm$core$Dict$remove, requestId, model.e),
										k: sourceData
									}));
						} else {
							return A4(
				...
				case 2:
					var samplesPerSecond = response.a.E;
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								E: $elm$core$Maybe$Just(samplesPerSecond)
							}),
...
var $MartinSStewart$elm_audio$Audio$offsetBy = F2(
	function (offset_, audio_) {
		return $MartinSStewart$elm_audio$Audio$Effect(
			{
				a5: audio_,
				ak: $MartinSStewart$elm_audio$Audio$Offset(offset_)
			});
	});
		...
		return $elm$browser$Browser$element(
			{
				be: A2(
					$elm$core$Basics$composeR,
					app.be,
					A2($MartinSStewart$elm_audio$Audio$initHelper, app.a6.bk, app.a5)),
				bj: $MartinSStewart$elm_audio$Audio$subscriptions(app),
				bl: $MartinSStewart$elm_audio$Audio$update(app),
				bm: function (model) {
					return A2(
						$elm$html$Html$map,
						$MartinSStewart$elm_audio$Audio$UserMsg,
...
var $author$project$Main$LoadingModel = {$: 0};
...
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple3(
		...
		$elm$core$Platform$Cmd$none,
		A2($MartinSStewart$elm_audio$Audio$loadAudio, $author$project$Main$SoundLoaded, './sin440.wav'));
};
...
var $author$project$Main$GetStartTimestamp = function (a) {
	return {$: 7, a: a};
};
var $author$project$Main$LoadFailedModel = {$: 2};
...
var $author$project$Main$NotPlaying = {$: 0};
...
var $author$project$Main$StopAudio = {$: 12};
...
var $author$project$Main$setSystemTime = A2($elm$core$Task$perform, $author$project$Main$SetSystemTime, $elm$time$Time$here);
							...
							$author$project$Main$LoadedModel(
								{
									...
									t: _List_Nil,
									as: sound,
									_: $author$project$Main$NotPlaying,
									ab: $elm$time$Time$millisToPosix(0),
									ag: 0,
									ai: $elm$time$Time$utc
								}),
				...
				switch (msg.$) {
					...
					case 4:
						...
						return _Utils_Tuple3(
								...
								_Utils_update(
									loadedModel,
									{
										t: A2(
											$elm$core$List$map,
											function (sample) {
												return _Utils_eq(sample.P, time) ? _Utils_update(
													sample,
													{W: input}) : sample;
											},
											loadedModel.t)
							...
							$MartinSStewart$elm_audio$Audio$cmdNone);
					case 5:
						var _v5 = $elm$core$String$toInt(loadedModel.L);
						if (!_v5.$) {
							var min = _v5.a;
							var _v6 = $elm$core$String$toInt(loadedModel.K);
							if (!_v6.$) {
								var max = _v6.a;
								return _Utils_Tuple3(
					...
					case 6:
						...
						return _Utils_Tuple3(
							$author$project$Main$LoadedModel(
								_Utils_update(
									loadedModel,
									{aj: newFace * 1000, ag: 0})),
							A2($elm$core$Task$perform, $author$project$Main$GetStartTimestamp, $elm$time$Time$now),
							$MartinSStewart$elm_audio$Audio$cmdNone);
					case 7:
						var timestamp = msg.a;
						return _Utils_Tuple3(
						...
						return _Utils_Tuple3(
							$author$project$Main$LoadedModel(
								_Utils_update(
									loadedModel,
									{
										_: $author$project$Main$Playing(time)
									})),
					...
					case 9:
						var now = msg.a;
						return loadedModel.s ? ((_Utils_cmp(
							$elm$time$Time$posixToMillis(now) - $elm$time$Time$posixToMillis(loadedModel.ab),
							loadedModel.aj) < 0) ? _Utils_Tuple3(
							$author$project$Main$LoadedModel(
								_Utils_update(
									loadedModel,
									{
										ag: (($elm$time$Time$posixToMillis(now) - $elm$time$Time$posixToMillis(loadedModel.ab)) / 1000) | 0
									})),
							$elm$core$Platform$Cmd$none,
...
var $author$project$Main$MaxTimeInput = function (a) {
	return {$: 3, a: a};
};
var $author$project$Main$MinTimeInput = function (a) {
	return {$: 2, a: a};
};
...
var $author$project$Main$SampleInput = F2(
	function (a, b) {
		return {$: 4, a: a, b: b};
	});
...
var $author$project$Main$sampleList = F2(
	function (samples, zone) {
		return A2(
			$elm$html$Html$ul,
			_List_Nil,
			A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										$elm$html$Html$text(
										A2($author$project$Main$formatTime, sample.P, zone))
									])),
								A2(
								$elm$html$Html$input,
								_List_fromArray(
									[
										...
										$elm$html$Html$Attributes$value(sample.W),
										$elm$html$Html$Events$onInput(
										$author$project$Main$SampleInput(sample.P))
									]),
								_List_Nil)
							]));
				},
...
var $author$project$Main$StartTimer = {$: 5};
...
var $author$project$Main$startButton = F2(
	function (min, max) {
		var _v0 = $elm$core$String$toInt(min);
		if (!_v0.$) {
			var _v1 = $elm$core$String$toInt(max);
			if (!_v1.$) {
				return A2(
...
var $author$project$Main$StopTimer = {$: 8};
var $author$project$Main$stopButton = A2(
	$elm$html$Html$button,
	_List_fromArray(
		[
			$elm$html$Html$Events$onClick($author$project$Main$StopTimer)
		]),
	_List_fromArray(
		[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromInt(loadedModel.ag))
								])),
							A2(
							$elm$html$Html$input,
							_List_fromArray(
								[
									$elm$html$Html$Attributes$value(loadedModel.L),
									$elm$html$Html$Attributes$placeholder('min'),
									$elm$html$Html$Events$onInput($author$project$Main$MinTimeInput)
								]),
							_List_Nil),
							A2(
							$elm$html$Html$input,
							_List_fromArray(
								[
									$elm$html$Html$Attributes$value(loadedModel.K),
									$elm$html$Html$Attributes$placeholder('max'),
									$elm$html$Html$Events$onInput($author$project$Main$MaxTimeInput)
								]),
							_List_Nil),
								...
								[
									...
									A2(
										...
										A2(
											$elm$core$List$map,
											function (sample) {
												return A2($author$project$Main$formatTime, sample.P, loadedModel.ai) + (',' + sample.W);
											},
											loadedModel.t)))
								]))
						]));
...
var $author$project$Main$main = $MartinSStewart$elm_audio$Audio$elementWithAudio(
	{
		a5: $author$project$Main$audio,
		a6: {bb: $author$project$Main$audioPortFromJS, bk: $author$project$Main$audioPortToJS},
		be: $author$project$Main$init,
		bj: $author$project$Main$subscriptions,
		bl: $author$project$Main$update,
		bm: $author$project$Main$view
	});
